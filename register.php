        ...
        if(!empty($firstname) && !empty($lastname) && !empty($email) && !empty($mobilenumber) && !empty($password) && !empty($repassword) && !empty($address) && !empty($address2) && !empty($city) && !empty($state) && !empty($zipcode)){
            ...
            if($rowCount > 0) {
                ...
                $Allowed_Chars='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789./';
                    ...
                    // Query
                    $sql = "INSERT INTO persons (role,fname, lname, email, phone, password_hash, password_salt, address, address2, city, state, zip_code) VALUES ('user', '{$firstname}', '{$lastname}', '{$email}', '{$mobilenumber}', '{$password_hash}', '{$bcrypt_salt}','{$address}', '{$address2}', '{$city}','{$state}','{$zipcode}')";
